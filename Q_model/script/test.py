...
checkpoint_path = path + "/778Q1mdn_checkpoint_best.pth"
...
def train(epoch, loader, config, partition='test', loss_type='mdn'):
    res = {'loss': 0, 'counter': 0, 'loss_arr': []}
    train_npred, train_ntrue = [], []
    valid_npred, valid_ntrue = [], []
    test_npred, test_ntrue = [], []
    ...
    for i, data in enumerate(loader):
        ...
        pi, sigma, mu = model(h=nodes, x=atom_positions, edges=edges, edge_attr=edge_attr,
                              node_mask=atom_mask, n_nodes=n_nodes, adapter=config.train.property)
    ...
    if partition == 'train':
        ...
        rmse = np.sqrt(mean_squared_error(mad * train_pred + meann, train_true))
        mae = mean_absolute_error(mad * train_pred + meann, train_true)
        r2 = r2_score(train_true, mad * train_pred + meann)

    elif partition == 'valid':
        valid_true = np.concatenate(np.array(valid_ntrue), 0)
        valid_pred = np.concatenate(np.array(valid_npred), 0)
        rmse = np.sqrt(mean_squared_error(mad * valid_pred + meann, valid_true))
        mae = mean_absolute_error(mad * valid_pred + meann, valid_true)
        r2 = r2_score(valid_true, mad * valid_pred + meann)

    else:
        test_true = np.concatenate(np.array(test_ntrue), 0)
        test_pred = np.concatenate(np.array(test_npred), 0)
        rmse = np.sqrt(mean_squared_error(mad * test_pred + meann, test_true))
        mae = mean_absolute_error(mad * test_pred + meann, test_true)
        r2 = r2_score(test_true, mad * test_pred + meann)
...
if __name__ == "__main__":
    ...
    test_loss, test_rmse, test_mae, test_r2 = train(0, dataloaders['test'], config, partition='test')
